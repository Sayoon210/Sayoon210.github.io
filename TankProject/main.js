...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Main game camera (tank follow camera)
    const gameCamera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000);
    gameCamera.position.set(0, 10, 15); // Initial position for game camera

    // OrbitControls camera (자유 시점 카메라)
    const orbitCamera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000);
    orbitCamera.position.set(20, 20, 20); // Initial position for orbit camera

    let activeCamera = gameCamera; // 시작은 탱크 추적 카메라 (gameCamera)
    ...
    // --- OrbitControls Setup for gameCamera (이름 gameControls로 변경) ---
    // 이 컨트롤은 gameCamera에 연결되지만, 실제 제어는 코드에서 직접 하므로 enabled는 false로 유지합니다.
    const gameControls = new OrbitControls(gameCamera, renderer.domElement);
    gameControls.enableDamping = true;
    gameControls.dampingFactor = 0.05;
    gameControls.target.set(0, 0, 0); // 카메라가 바라볼 지점 설정
    gameControls.update();
    gameControls.enabled = false; // gameCamera는 코드로 제어되므로 OrbitControls는 비활성화

    // --- OrbitControls Setup for orbitCamera ---
    const orbitControls = new OrbitControls(orbitCamera, renderer.domElement);
    orbitControls.enableDamping = true;
    orbitControls.dampingFactor = 0.05;
    orbitControls.target.set(0, 0, 0); // 초기 목표는 월드 원점
    orbitControls.update();
    orbitControls.enabled = false; // 처음에는 비활성화
    ...
    window.addEventListener('keydown', (event) => {
        ...
        if (event.key === '0') keys.zero = true; // 0 키 추가
    });
    ...
    window.addEventListener('keyup', (event) => {
        ...
        if (event.key === '0') keys.zero = false; // 0 키 추가
    });
    ...
    // --- Camera Switching Logic Variables ---
    let lastZeroPressTime = 0;
    const cameraSwitchCooldown = 500; // 0 키 입력 쿨다운 (밀리초)

    // --- HTML Instructions Text ---
    const instructionsDiv = document.getElementById('instructions');
    if (instructionsDiv) {
        instructionsDiv.innerHTML = `
            1. Press W / S to move front / back<br>
            2. Press A / D to rotate counter clock / clock direction<br>
            3. Press Q / E to rotate Turret<br>
            4. Press R / F to adjust the Barrel<br>
            5. Press Space to Raycast cannon<br>
            6. Press Number '0' to use Orbit Camera
        `;
    } else {
        console.error("Instructions div not found in HTML!");
    }
    ...
    RAPIER.init().then(() => {
        ...
        const tankModelPath = 'tankv3.glb';
        ...
        tankLoader.load(
            ...
            function (gltf) {
                ...
                poshinSmall = tankMesh.getObjectByName('poshinSmall'); // 소문자 'p'로 수정
                if (poshinSmall) { console.log("PoshinSmall mesh found:", poshinSmall); } else { console.warn("PoshinSmall mesh not found!"); }
    ...
    function animate() {
        ...
        // deltaTime for raycast duration (not for physics step, which Rapier handles internally)
        // const deltaTime = (currentTime - raycastTimer) / 1000; // This was used for raycast timer, but it's not actually deltaTime for physics step
        raycastTimer = currentTime; // Update raycastTimer for next frame's comparison

        // Camera switching logic
        if (keys.zero && (currentTime - lastZeroPressTime > cameraSwitchCooldown)) {
            lastZeroPressTime = currentTime;
            if (activeCamera === gameCamera) {
                activeCamera = orbitCamera;
                gameControls.enabled = false; // 게임 카메라는 더 이상 마우스로 조작하지 않음
                orbitControls.enabled = true; // OrbitControls 활성화

                // 전환 시 orbitCamera의 위치를 gameCamera의 현재 위치로 설정하여 부드러운 전환을 돕습니다.
                orbitCamera.position.copy(gameCamera.position);
                orbitControls.target.copy(gameControls.target); // gameControls의 목표 지점 사용 (탱크 위치)
                orbitControls.update(); // OrbitControls 내부 상태 업데이트
                console.log(`Switched to Orbit Camera. Position: ${orbitCamera.position.x.toFixed(2)}, ${orbitCamera.position.y.toFixed(2)}, ${orbitCamera.position.z.toFixed(2)}`);
            } else {
                activeCamera = gameCamera;
                gameControls.enabled = false; // 게임 카메라는 여전히 코드로 제어
                orbitControls.enabled = false; // OrbitControls 비활성화
                console.log(`Switched to Game Camera. Position: ${gameCamera.position.x.toFixed(2)}, ${gameCamera.position.y.toFixed(2)}, ${gameCamera.position.z.toFixed(2)}`);
            }
        }
        ...
        if (world) {
            ...
            if (tankRigidBody && tankMesh) {
                ...
                // const angularBrakingImpulse = 20000; // 현재 사용되지 않음
                ...
                // Only apply tank movement, turret, and raycast controls if gameCamera is active
                if (activeCamera === gameCamera) {
                    if (keys.w) {
                        applyBraking = false;
                    ...
                    // Potap (포탑) 회전 로직
                    if (potapMesh) { // activeCamera === gameCamera 조건은 이미 상위 if문에서 검사됨
                    ...
                    if (poshinRotateCore) { // activeCamera === gameCamera 조건은 이미 상위 if문에서 검사됨
                    ...
                    if (keys.space && !raycastActive && (performance.now() - lastSpacePressTime > raycastCooldown)) {
                        if (poshinBig && poshinSmall) { // PoshinRotateCore와 PoshinBig 모두 로드되었는지 확인
                            lastSpacePressTime = performance.now();

                            const origin = new THREE.Vector3();
                            poshinBig.getWorldPosition(origin); // Raycast 시작점: PoshinRotateCore의 월드 위치

                            const targetPoint = new THREE.Vector3();
                            poshinSmall.getWorldPosition(targetPoint); // Raycast 목표점: PoshinBig의 월드 위치
                    ...
                    if (tankMesh && potapMesh) {
                        ...
                        // gameCamera의 위치를 lerp로 부드럽게 이동
                        gameCamera.position.lerp(targetCameraPosition, cameraSmoothFactor);
                        ...
                        // gameCamera가 바라볼 지점 설정
                        gameCamera.lookAt(targetLookAtPoint);

                        // gameControls의 target도 업데이트하여 전환 시 부드럽게 합니다.
                        gameControls.target.copy(targetLookAtPoint);
                        // gameControls.update(); // 이 부분은 이제 gameControls.enabled가 false이므로 필요하지 않습니다.
                        // 하지만 전환 시 target 값을 유지하기 위해 update()를 호출해도 무방합니다.
                        // 여기서는 gameCamera.lookAt()으로 직접 제어하므로 주석 처리합니다.
                    }
                } // End of if (activeCamera === gameCamera)
            }
        }

        // 활성 카메라에 따라 컨트롤러 업데이트
        if (activeCamera === orbitCamera) {
            orbitControls.update();
        }
        // gameControls는 항상 update()를 호출하여 내부 상태를 유지합니다.
        // enabled가 false이므로 사용자의 마우스 입력은 받지 않습니다.
        gameControls.update();


        renderer.render(scene, activeCamera); // 현재 활성 카메라로 렌더링
    }
